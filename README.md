...
`§b§l§nПредложить идею ` – Позволяет отправить предложение по улучшению CClient напрямую разработчику. Идея вводится в специальном окне и отправляется через Discord вебхук.
...
*   Привязывать модули к клавишам: для этого кликните правой кнопкой мыши (ПКМ) по названию модуля в списке, а затем нажмите желаемую клавишу. Нажатие ESC во время привязки отменит ее.
...
*   `.inblock <сила>`: устанавливает силу (количество пакетов) для модулей InBlock/LagClip.
...
*   **Сохранение конфигураций**: Все настройки модулей, интерфейса и других систем клиента сохраняются, чтобы вам не приходилось настраивать все заново при каждом запуске. Существует "гусиная пасхалка", активируемая командой `.goosepashalka` в меню ClickGUI.
